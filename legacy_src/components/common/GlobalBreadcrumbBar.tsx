...
export const GlobalBreadcrumbBar = () => {
  ...
  useEffect(() => {
    ...
    setHistoryItems(prev => {
      ...
      if (last?.pathname === location.pathname) return prev;

      const next = [...prev, { pathname: location.pathname, label }];
  ...
  const breadcrumbItems = useMemo(() => {
    if (historyItems.length === 0) {
      return [{ pathname: location.pathname, label: getLabelForPath(location.pathname) }];
    }
    return historyItems.slice(-3);
  }, [historyItems, location.pathname]);
